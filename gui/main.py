...
def ShowMeTheButtons():
    ...
    layout = [[sg.Frame('Connection to Device', device_selection, font=('any 18'), background_color='black'),
               sg.Frame('Device Controls', device_buttons, font=('any 18'), background_color='black')]]
...
def sleep_handler(window):
    ...
    if connected:
        if sleep:
            result = WakeUp()
            window.Element('_wakeup_').Update("Sleep", image_data=image_file_to_bytes(green_pill64, (100, 50)))
            sleep = not sleep
        else:
            result = Sleep()
            window.Element('_wakeup_').Update("Wake Up", image_data=image_file_to_bytes(red_pill64, (100, 50)))
            sleep = not sleep
    else:
        print("Not connected")
